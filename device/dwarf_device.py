...
class Dwarf:
    def __new__(cls, *args, **kwargs):
        print("Create a new instance of Dwarf.")
        return super().__new__(cls)
        
    # <ip_address> <port> <device name> <device num>
    def __init__(self, logger, host, port, device_name, device_num, is_debug = False):
        print(f"Initialize the new instance of Dwarf: {host}:{port}, name:{device_name}, num:{device_num}, is_debug:{is_debug}")
        ...
        self.below_horizon_dec_offset = 0 # we will use this to work around below horizon. This value will ve used to fool dwarf's star map
...
if __name__ == '__main__':
    try:
        if len(sys.argv) != 5:
            ...
            sys.exit()
        dwarf_core = Dwarf(sys.argv[1], int(sys.argv[2]), sys.argv[3], int(sys.argv[4]), is_debug = True)
        dwarf_core.start_watch_thread()
        request = {}
        request['method'] = 'iscope_stop_view'
        print("sync response: ", dwarf_core.send_message_param_sync(request))
        time.sleep(10)
        dwarf_core.end_watch_thread()
...
def main():
    ...
    print("dwarf_run version: ", version_string)
    
    if len(sys.argv) != 11 and len(sys.argv) != 12:
        print("expected dwarf_run <ip_address> <target_name> <ra> <dec> <is_use_LP_filter> <session_time> <RA panel size> <Dec panel size> <RA offset factor> <Dec offset factor>")
        sys.exit()
...
# dwarf_run <ip_address> <target_name> <ra> <dec> <is_use_LP_filter> <session_time> <RA panel size> <Dec panel size> <RA offset factor> <Dec offset factor>
# python dwarf_run.py 192.168.110.30 'Castor' '7:24:32.5' '-41:24:23.5' 0 60 2 2 1.0 1.0
# python dwarf_run.py 192.168.110.30 'Castor' '7:24:32.5' '+41:24:23.5' 0 60 2 2 1.0 1.0
# python dwarf_run.py 192.168.110.30 'Castor' '7:24:32.5' '41:24:23.5' 0 60 2 2 1.0 1.0
# python dwarf_run.py 192.168.110.30 'Castor' 7.4090278 41.4065278 0 60 2 2 1.0 1.0
if __name__ == "__main__":
    try:
        main()
    except KeyboardInterrupt:
